...
/**
 * Routes listing page that displays all route blogs (cityToCity) in an iframe
...
export default function RoutesPage() {
  ...
  // URL to the external routes listing page
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      // Handle navigation to individual route posts
      if (event.data.type === 'chiri-payload' && event.data.payload?.url) {
        console.log('Navigating to route:', event.data.payload.url)
      ...
      // Handle navigation back to routes listing
      if (event.data.type === 'chiri-routes-button-click') {
        // Already on routes page, no navigation needed
        console.log('Already on routes page')
      }
