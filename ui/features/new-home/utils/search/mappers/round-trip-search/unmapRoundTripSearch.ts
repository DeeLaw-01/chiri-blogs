...
export function useUnmapRoundTripSearch () {
  const searchParams = useSearchParams()
  const query = Object.fromEntries(searchParams.entries())
  const hasExecuted = useRef<boolean>(false)
  const { setRoundtripSearch, setRoundtripFilters } = useSearchAtoms()
  ...
  useEffect(() => {
    if (
      query.type !== HomeState.ROUNDTRIP ||
      isLoading ||
      !selectedCity ||
      avoidIsLoading ||
      hasExecuted.current
    ...
    handleSetRoundTripSearch()
    handleSetRoundTripFilters()
  }, [isLoading, selectedCity, avoidIsLoading])

  const handleSetRoundTripSearch = () => {
    setRoundtripSearch({
      ...getFiltersOrSearchParams(true),
      categories: 'single-city', // Always set to single-city for round trips
  ...
  const handleSetRoundTripFilters = () => {
    setRoundtripFilters({
