...
/**
 * Proxy API route for airlines
 * 
 * This route forwards requests from Site B to Site A's airlines-list API.
 * All airline data management happens in Site A's MongoDB.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const siteAUrl = `${SITE_A_BASE_URL}/api/airlines-list?${searchParams.toString()}`
    ...
    if (!response.ok) {
      ...
      console.error('[Proxy] Site A airlines-list API error:', response.status, errorText)
      
      return NextResponse.json(
        { 
          error: 'Failed to fetch airlines from Site A',
          details: errorText,
    ...
    console.error('[Proxy] Error forwarding airlines-list request:', errorMessage)
    
    return NextResponse.json(
      { 
        error: 'Failed to proxy airlines-list request',
