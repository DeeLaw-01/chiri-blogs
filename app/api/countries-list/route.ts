...
/**
 * Proxy API route for countries
 * 
 * This route forwards requests from Site B to Site A's countries-list API.
 * All country data management happens in Site A's MongoDB.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const siteAUrl = `${SITE_A_BASE_URL}/api/countries-list?${searchParams.toString()}`
    ...
    if (!response.ok) {
      ...
      console.error('[Proxy] Site A countries-list API error:', response.status, errorText)
      
      return NextResponse.json(
        { 
          error: 'Failed to fetch countries from Site A',
          details: errorText,
    ...
    console.error('[Proxy] Error forwarding countries-list request:', errorMessage)
    
    return NextResponse.json(
      { 
        error: 'Failed to proxy countries-list request',
