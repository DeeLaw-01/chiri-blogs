...
export default function ArrivalStationsFilter() {
  ...
  const { stationsError, getArrivalStations, isLoading } = useStations(
    origin,
    destination,
  ...
  if (getArrivalStations().length === 0)
    ...
    <RadioGroup
      onChange={(e) =>
        setOnewayFilters((prev) => ({
          ...prev,
          specific_destination_station: e,
        }))
      }
      value={onewayFilters.specific_destination_station}
    >
      <SimpleGrid columns={[2, 2, 3]} spacing={4} alignItems="flex-start">
        <Radio value={''}>
          <Text fontSize="sm" st={'common.general.any'} />
        </Radio>
        {getArrivalStations()?.map((item, idx) => {
          return (
